...
export default function TopTrendsRankings({ title, subheader, list, ...other }: Props) {
    ...
    const orderList = (items: ItemProps[]): ItemProps[] => {
        // Ordenamos primero los valores positivos de mayor a menor
        const positives = orderBy(items.filter(item => item.type === 'POS'), ['change'], ['desc']);
        // Luego ordenamos los valores negativos de menor a mayor
        const negatives = orderBy(items.filter(item => item.type === 'NEG'), ['change'], ['asc']);
        // Retornamos la concatenación de los positivos seguidos de los negativos
        return [...positives, ...negatives];
    };


    const sortedList = orderList(list);
    const itemsToShow = expanded ? sortedList : sortedList.slice(0, 5);
    ...
    return (
        <Card {...other}>
            ...
            <Stack spacing={3} sx={{ p: 3 }}>
                {itemsToShow.slice(0, expanded ? 10 : 3).map((author, index) => (
